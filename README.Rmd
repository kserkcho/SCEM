...
The first method estimates input parameters for use with a previously-developed parametric approach (Tornero et al., 2013). The relevant code for this approach is `makeFits_OLS.R`, while `makeFits_initial.R` is the code to implement the same method but with given initial conditions for two parameters. \code{makeFits(paths,method="OLS")} and \code{makeFits(paths,method="initial")} can be used to use respective methods. The latter can be used to show the disadvantage of the existing approach.
